...
function isPalindrome(str) {
  ...
  const chars = [];
  for (let i = 0; i < strLc.length; i += 1) {
    const charCode = strLc[i].charCodeAt(0);
...
function invertCase(str) {
  return str
    .split('')
    .map((value) =>
      value.charCodeAt(0) <= 90 ? value.toLowerCase() : value.toUpperCase()
    )
    .join('');
}
  ...
  return chars
    .map((char) => {
      if (alphabet.includes(char.toLowerCase())) {
        const char13 =
          alphabet[(alphabet.indexOf(char.toLowerCase()) + 13) % 26];
        return char >= 'a' ? char13 : char13.toUpperCase();
      }
      return char;
    })
    .join('');
...
function getCardId(value) {
  ...
  const rank = ranks.indexOf(value.slice(0, value.length - 1));
  const suit = suits.indexOf(value.at(-1));

  return ranks.length * suit + rank;
}
